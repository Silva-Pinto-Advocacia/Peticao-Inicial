...
def process_zip(zip_path: Path, session_dir: Path, form_data: dict, api_key: str) -> dict:
    ...
    # 2. Collect document content for Claude
    documents_content = []
    docx_model_path   = None
    text_summary      = []

    text_summary.append("=== INVENTÁRIO DO ZIP ===")
    for f in file_list:
        text_summary.append(f"  • {f}")
    ...
    for fpath in all_files:
        ...
        if name.endswith(".docx"):
            # Check if it's the modelo
            if "modelo" in name or "peticao" in name or "inicial" in name:
                docx_model_path = fpath
            ...
            text_summary.append(f"\n--- PDF enviado ao Claude: {rname} ---")

        elif name.endswith((".xlsx", ".csv", ".txt")):
            text = read_file_text(fpath)
            text_summary.append(f"\n--- {rname} ---\n{text}")

    # Fallback: any docx as modelo
    if not docx_model_path:
        for fpath in all_files:
            if fpath.is_file() and fpath.suffix.lower() == ".docx":
                docx_model_path = fpath
                break
...
Analise todos os documentos acima e retorne o JSON conforme o schema do sistema.
"""

    log.info("Calling Claude API...")
    documents_content.insert(0, {"type": "text", "text": user_message})
